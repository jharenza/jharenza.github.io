...
#### New columns: `pathology_free_text_diagnosis` and `cohort_participant_id` and `glioma_brain_region` -> `CNS_region`
```{r}
dim(v16_hist)
dim(v17_hist)
...
```
#### Sample and aliquot IDs `r emo::ji("x")` - PNOC003 `BS_MVYA262V` sample `7316-3213` was changed from `A05265` to `A05233`. Is this correct? `r emo::ji("exclamation")`
```{r}
